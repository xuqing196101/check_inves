...
<!-- 项目戳开始 -->
   ...
   <div class="container container_box">
      <div>
        <h2 class="count_flow"><i>1</i>计划基本信息</h2>
        <ul class="ul_list">
          <table class="table table-bordered mt10">
            <tbody>
              <tr>
                <td width="25%" class="info">计划名称：</td>
                <td width="25%">${plan.fileName}</td>
                <td width="25%" class="info">计划编号：</td>
                <td width="25%">${plan.planNo}</td>
              </tr>
              <tr>
                <td width="25%" class="info">计划状态：</td>
                <td width="25%">
				<c:if test="${plan.status == 1}">审核轮次设置</c:if>
                <c:if test="${plan.status == 2}">已下达</c:if>
                <c:if test="${plan.status == 3}">第一轮审核</c:if>
                <c:if test="${plan.status == 4}">第二轮审核人员设置</c:if>
                <c:if test="${plan.status == 5}">第二轮审核</c:if>
                <c:if test="${plan.status == 6}">第三轮审核人员设置</c:if>
                <c:if test="${plan.status == 7}">第三轮审核</c:if>
                <c:if test="${plan.status == 12}">未下达</c:if>

                 </td>
                 <td width="25%" class="info">创建人：</td>
                <td width="25%">${plan.userId}</td>
              </tr>
              <tr>
