...
<c:forEach items="${lists }" var="obj" varStatus="vs">
	<tr name="detailRow">
		...
		<td><input type='hidden' value='${obj.id }'> <input
			...
			name="list[${vs.index }].budget" value="${obj.budget}"> <input
			type="hidden" value="${obj.parentId }"></td>


		<td><input type="text" class="deliverdate"
			name="list[${vs.index }].deliverDate" value="${obj.deliverDate}"></td>
		<td><select name="list[${vs.index }].purchaseType"
			class="purchasetype" onchange="changeType(this)">
				<option value="">请选择</option>
				<c:forEach items="${list2 }" var="objd">
					<option value="${objd.name}"
						<c:if test="${objd.name==obj.purchaseType}"> selected='selected'</c:if>>
						${objd.name }</option>

				</c:forEach>
		</select></td>
		<td><input type="text" name="list[${vs.index }].supplier" onblur="checkSupplierName('${vs.index }')"
			class="m0 w260 border0" value="${obj.supplier}"> <%-- <select name="list[${vs.index }].supplier" class="purchasename" onchange="changeType(this)" id="pType[0]">
												<option value="">请选择</option>
		...
		<td name='userNone'><input type="text"
			name="list[${vs.index }].useUnit" class="useunit"></td>
