  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      function changeType(obj) {
        var dyly = "${DYLY}";
        var price = $(obj).parent().prev().prev().prev().children(".price").val();
        if(price){
          var purchaseType = $(obj).find("option:selected").text();
          ...
          for(var i = 0; i < next.length; i++) {
            ...
            if($(obj).val() == dyly) {
              var prices = $($(next[i]).children()[7]).children(".price").val();
              if(prices){
            ...
            <form id="adjust" action="${pageContext.request.contextPath}/adjust/updatePlan.html" method="post">
              <c:forEach items="${list}" var="obj" varStatus="vs">
                <tr>
                  ...
                  <td>
                    <input type="hidden" name="ss" value="${obj.id}" />
                    <select name="list[${vs.index }].purchaseType" onchange="changeType(this);" class="purchasetype" id="select">
                      <option value="">请选择</option>
                      <c:forEach items="${types}" var="mt">
                        <option value="${mt.id}" <c:if test="${mt.id eq obj.purchaseType }"> selected="selected"</c:if>>${mt.name}</option>
                      </c:forEach>
                    </select>
