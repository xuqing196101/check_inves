	...
	<head>
		...
		<script type="text/javascript">
    	   ...
    	   $(".org option").each(function(){
			...
			function splitRemoveType(index, tdVal2, histVal, eq) {
				var old = $("#audit_table tr:eq(" + index + ")").find("td:eq(" + eq + ")").children(":first").val();
				var oldSplit = old.split("；");
				var count = 0;
				var fg = true;
				if(tdVal2 == "采购方式") {
					is: for(var i = 0; i < oldSplit.length; i++) {
						if(oldSplit[i].indexOf("采购方式") >= 0) {
							var ts2 = oldSplit[1].substring(oldSplit[1].indexOf("成") + 1, oldSplit[1].length);
							if(ts2 != "单一来源") {
								if(old.indexOf("供应商名称") >= 0) {
									js: for(var j = 0; j < oldSplit.length; j++) {
										if(oldSplit[j].indexOf("供应商名称") >= 0) {
											count = j;
											fg = false;
											break js;
										}
				...
				if(!fg) {
					removeByValue(oldSplit, oldSplit[count]);
				}
				for(var h = 0; h < oldSplit.length; h++) {
					if(oldSplit[h] != "") {
						if(oldSplit[h].indexOf(tdVal2 + "：由" + histVal + "变成") != -1) {
			...
			function change(obj) {
				...
				if(val != defVal) {
					...
					if(status == '3') {
						...
						var newVal = splitS(inpval, defVal, curval, val, tdVal1);
			...
			function typeChange(obj) {
				...
				if( price!="") {
					...
					var nexts = $(obj).parent().parent().nextAll();
					...
					for(var i = 0; i < nexts.length; i++) {
						if(parent_id == $($(nexts[i]).children()[10]).children(":last").val()) {
							break;
						}
						$($(nexts[i]).children()[10]).children(":first").next().val($(obj).val());
						if($(obj).val() == "26E3925D38BB4295BEB342BDC82B65AC") {
							$($(nexts[i]).children()[12]).find("input").removeAttr("readonly");
						} else {
							$($(nexts[i]).children()[12]).find("input").val("");
							$($(nexts[i]).children()[12]).find("input").attr("readonly", "readonly");
						}
					}
				}
				var id = $(obj).prev().val();
										...
										if(org == opt) {
											...
											if(org != defValue) {
												...
												if(status == '3') {
													var inpval = $("#audit_table tr:eq(" + index + ")").find("td:eq(0)").children(":first").val();
													var curval = tdVal1 + "由" + defVal + "变成" + val;
													var newVal = splitType(inpval, defVal, curval, val, tdVal1);
													$("#audit_table tr:eq(" + index + ")").find("td:eq(0)").children(":first").val(newVal);
												}
												if(status == '5') {
													var inpval = $("#audit_table tr:eq(" + index + ")").find("td:eq(1)").children(":first").val();
												...
												splitRemoveType(index, tdVal1.substring(0, tdVal1.length - 1), defVal, 0);
			...
			function splitS(inpval, defVal, curval, val, tdVal1) {
				var newVal = "";
				if(inpval.length > 0) {
					var flg = false;
					var str = inpval.split("；");
					for(var i = 0; i < str.length; i++) {
						if(str[i].indexOf(tdVal1 + "由" + defVal + "变成") >= 0) {
							str[i] = curval;
							flg = true;
							break;
						}
			...
			function splitType(inpval, defVal, curval, val, tdVal1) {
				var newVal = "";
				if(inpval.length > 0) {
					var flg = false;
					var str = inpval.split("；");
					...
					if(flg) {
						...
						var count = 0;
						var fg = true;
						if(newVal.indexOf("采购方式") >= 0) {
							is: for(var i = 0; i < str.length; i++) {
								if(str[i].indexOf("采购方式") >= 0) {
									var str1 = str[i].split("：");
									var ts2 = str1[1].substring(str1[1].indexOf("成") + 1, str1[1].length);
									if(ts2 != "单一来源") {
										if(newVal.indexOf("供应商名称") >= 0) {
											js: for(var j = 0; j < str.length; j++) {
												if(str[j].indexOf("供应商名称") >= 0) {
													count = j;
													fg = false;
													break js;
												}
