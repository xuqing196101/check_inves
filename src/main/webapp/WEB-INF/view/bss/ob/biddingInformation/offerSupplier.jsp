      ...
      jump : function(e, first) { //触发分页后的回调
    	 if(!first) { //一定要加此判断，否则初始时会无限刷新
			$("#page").val(e.curr);
    	 	...
    	 	window.location.href = "${pageContext.request.contextPath}/ob_project/offerSupplierList.html?page=" + e.curr +"&&obProjectId="+obProjectId+"&&name="+supname+"&&status="+status;
		}
      }
    });
  });
...
function resetQuery() {
	...
	var obProjectId = $("#obProjectId").val();
 	window.location.href = "${pageContext.request.contextPath}/ob_project/offerSupplierList.html?obProjectId="+obProjectId;
}
...
<!-- 供应商列表页面开始 -->
    ...
    <div class="search_detail">
       <form action="${pageContext.request.contextPath}/ob_project/offerSupplierList.html" method="post" class="mb0" id = "form1">
    	<input id = "obProjectId" name = "obProjectId" value = "${obProjectId }" style="display: none;">
    	<ul class="demand_list">
    	<li>
	    	<label class="fl">供应商名称：</label>
			<input type="text" id="supname" class="" name = "name" value="${name }"/>
	     </li>
		<li>
