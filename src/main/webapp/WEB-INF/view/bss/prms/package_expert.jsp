			...
			<div class="container clear margin-top-30" id="package">
				...
				<c:forEach items="${packageList }" var="pack" varStatus="p">
					<form action="<%=basePath%>packageExpert/relate.html" method="post">
						...
						<input type="hidden" id="packageId" name="packageId"
							value="${pack.id }" /> <input type="hidden" name="projectId"
							value="${project.id}"> <input type="hidden"
							name="packageIds" id="packageIds">
						<div style="height: 110px; overflow: auto;">
							<table>

								<tr>
									<td
										style="white-space: nowrap; overflow: hidden; word-spacing: keep-all;">
										...
										<c:forEach items="${expertList }" var="expert" varStatus="vs">
											<input type="checkbox" name="chkItem"
												...
												<c:forEach items="${expertIdList }" var="e" varStatus="p">
														      	    	...
														      	    	</c:forEach>>${expert.expert.relName }&nbsp;
													      	</c:forEach> <!-- <input type="checkbox" name="chkItem" value="222">专家2
													      	<input type="checkbox" name="chkItem" value="333" onchange="clearSelect(this);">专家3 -->
										&nbsp;&nbsp; 组长：<select name="groupId"
											...
											<c:forEach items="${expertList }" var="expert" varStatus="vs">
												<option value="${expert.expert.id }"
													<c:forEach items="${expertIdList }" var="e" varStatus="p">
														      	    	...
														      	    	</c:forEach>>${expert.expert.relName }</option>
											</c:forEach>
									</select>&nbsp;&nbsp; <input type="button" onclick="submit1(this);"
										value="分配" class="btn btn-windows add">
									</td>
								</tr>
							</table>
						</div>
				...
				<c:forEach items="${packageList }" var="pack" varStatus="vs">
					<div align="right">
						...
						<button class="btn btn-windows input" onclick="window.print();"
							type="button">打印评分信息</button>
					</div>
					<table class="table table-bordered table-condensed mt5">
						<thead>
							<tr>
