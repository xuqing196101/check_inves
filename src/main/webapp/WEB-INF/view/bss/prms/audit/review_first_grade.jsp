	    ...
	    <table id="table" style="width:1600px; font-size: medium; max-width:10000px" class="table table-bordered lockout">
	      <thead>
			    <tr>
						...
						<th colspan="2" class="tc">${supplier.supplierName}</th>
						</c:forEach>
			    </tr>
			    <tr>
						<c:forEach items="${supplierList}" var="supplier">
    	      ...
    	      <td class="tc">
    	        <a href="javascript:void();" title='所 属 模 型 : ${model}&#10;评 审 指 标 : ${score.name}&#10;评 审 内 容 : ${score.reviewContent}'>
	 	        ...
	 	        <td class="p0">
		 	      ...
		 	      <c:otherwise>
		 	      <td class="p0">
	 	          <input type="text" title="单位:${score.unit}" name="expertValue" id="ipt5" onpaste="return false;" class="tc m0"
	 	            onchange="audit(this,'${score.id}','${supplier.supplierId}','${score.typeName}','${score.markTermId}','')"
	 	            <c:forEach items="${scores}" var="sco">
			...
			<div class="tc col-md-12 col-sm-12 col-xs-12 mt20 mb20">
