...
<html>
	...
	<head>
		...
		<script type="text/javascript">
			...
			function view(id, packageId, expertId, isGatherGather, isGather) {
				if (isGatherGather == 1 && isGather == 1){
					...
					$.ajax({
						...
						success: function(data) {
							if(data == '1') {
								toAudit(id, packageId);
							} else if(data == '2') {
								toFirstAudit(id, packageId);
							} else if(data == '3') {
								//进入基准价法和最低价法的经济技术评审
								window.location.href = "${pageContext.request.contextPath}/reviewFirstAudit/toCheck.html?projectId=" + id + "&packageId=" + packageId;
							} else {
								layer.msg(data);
							}
						}
		...
		<div class="margin-top-10 breadcrumbs ">
			<div class="container">
				<ul class="breadcrumb margin-left-0">
					...
					<li>
						<a href="javascript:void(0)">项目评审</a>
					</li>
				</ul>
				<div class="clear"></div>
			</div>
						...
						<tbody id="tbody_id">
							...
							<c:forEach items="${projectExtList.list}" var="obj" varStatus="vs">
								<tr style="cursor: pointer;">
									<td class="tc w50" onclick="view('${obj.id}','${obj.packageId}','${expertId}','${obj.packageExperts[0].isGatherGather}','${obj.packageExperts[0].isGather}')">${vs.count}</td>
									<td onclick="view('${obj.id}','${obj.packageId}','${expertId}','${obj.packageExperts[0].isGatherGather}','${obj.packageExperts[0].isGather}')">${obj.name}</td>
									<td onclick="view('${obj.id}','${obj.packageId}','${expertId}','${obj.packageExperts[0].isGatherGather}','${obj.packageExperts[0].isGather}')">${obj.projectNumber}</td>
									<td class="tc" onclick="view('${obj.id}','${obj.packageId}','${expertId}','${obj.packageExperts[0].isGatherGather}','${obj.packageExperts[0].isGather}')">${obj.packageName}</td>
									<td class="tc" onclick="view('${obj.id}','${obj.packageId}','${expertId}','${obj.packageExperts[0].isGatherGather}','${obj.packageExperts[0].isGather}')">
										<fmt:formatDate pattern="yyyy-MM-dd" value="${obj.bidDate}" />
									</td>
									<td class="tc">
										<c:forEach items="${obj.packageExperts}" var="pe">
											<c:if test="${pe.isAudit == 2 or pe.isAudit == 0}">资格性和符合性检查</c:if>
											<c:if test="${pe.isAudit == 1 && pe.isGather == 0}">资格性和符合性检查已提交</c:if>
											<c:if test="${(pe.isGrade == 0 and pe.isGather == 1 and pe.isAudit == 1) or (pe.isGrade == 2 and pe.isGather == 1 and pe.isAudit == 1)}">经济技术评审</c:if>
											<c:if test="${pe.isGrade == 1 and pe.isGather == 1 and pe.isGatherGather == 0}">经济技术评审已提交</c:if>
											<!-- 符合性审查结束，经济技术审查结束 -->
											<c:if test="${pe.isGatherGather == 1 and pe.isGather == 1}">评审结束</c:if>
										</c:forEach>
									</td>
								</tr>
							</c:forEach>
