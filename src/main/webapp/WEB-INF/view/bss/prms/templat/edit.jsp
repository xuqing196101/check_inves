	...
	<form action="<%=basePath %>auditTemplat/edit.html" method="post" id="form1">
     ...
     <thead>
      <tr>
        ...
        <th>初审项模板类型:</th><td><input type="checkbox" name="kind" value="商务" <c:if test="${fn:contains(templat.kind,'商务')}">checked="true"</c:if> >商务&nbsp;<input type="checkbox" <c:if test="${fn:contains(templat.kind,'技术')}">checked="true"</c:if> name="kind" id="kind" value="技术" >技术</td>
