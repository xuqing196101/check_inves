...
<html>
  <head>
	...
	<script type="text/javascript">
		...
		function placeArchive(){
			...
			if(count==0){
				layer.alert("请选择要归档的档案",{offset: ['30%', '40%']});
				$(".layui-layer-shade").remove();
				return;
			}
			for(var i=0;i<info.length;i++){    
			...
			$.ajax({
			    ...
			    success:function(data){
				    if(data==0){
				    	...
				    	layer.alert("请选择审核通过的档案归档",{offset: ['30%', '40%']});
						$(".layui-layer-shade").remove();
						return;
				    }
			    }
		...
		function git(){
			...
			if(count==0){
				layer.alert("请选择要提交的档案",{offset: ['30%', '40%']});
				$(".layui-layer-shade").remove();
				return;
			}
			for(var i=0;i<info.length;i++){    
					...
					url:"${pageContext.request.contextPath }/purchaseArchive/auditArchive.html?id="+str,
			       	success:function(data){
			       		if(data==0){
			       			...
			       			layer.alert("请选择已提交的档案审核",{offset: ['30%', '40%']});
							$(".layui-layer-shade").remove();
							return;
			       		}
			       	}
					...
					url:"${pageContext.request.contextPath }/purchaseArchive/judgeBorrow.html?id="+id,
			       	success:function(data){
			       		if(data==0){
			       			...
			       			layer.alert("对不起，您无权浏览本档案，请选择其它档案",{offset: ['30%', '40%']});
							$(".layui-layer-shade").remove();
							return;
			       		}
			       	}
  ...
  <body>
   ...
   <div class="container">
  	...
  	<%--<div class="container" id="addPurchase">
  		<button class="btn" type="button" onclick="addApp()">已审价的单一来源采购合同</button>
  		<button class="btn" type="button" onclick="addCom()">已验收的竞争性方式采购合同</button>
  	</div>
  --%></body>
</html>
