    ...
    function inputTemplete(){
        ...
        function exportWord(){
        	...
        	if(content == null || content == ""){
	            ...
	            $("#form").attr("action",'<%=basePath%>pub_tran/export.html');   
	            $("#form").submit();
        	}
        }
        //预览
        function preview(){
        	...
        	if(articleId == null || articleId == ""){
             ...
             $("#form").attr("action",'<%=basePath%>pub_tran/printView.html');   
             $("#form").submit();
        }
        //发布
        function publish(){
        	...
        	if(articleId == null || articleId == ""){
	            ...
	            layer.open({
	              ...
	              offset: '100px',
	              shadeClose: false,
	              content: '<%=basePath%>pub_tran/publishEdit.html?id='+articleId,
	              success: function(layero, index){
	                iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
	              }
	            });
        ...
        //    	$("#form").attr("action",'<%=basePath%>pub_tran/saveBidNotice.do?name='+text);   
        //        $("#form").submit();               
        //     });
       // }
       ...
       function save(){
       		$.ajax({
			    type: 'post',
			    url: "<%=basePath%>pub_tran/saveBidNotice.do",
			    dataType:'json',
			    data : $('#form').serialize(),
