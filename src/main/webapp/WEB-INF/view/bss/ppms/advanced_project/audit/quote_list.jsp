  ...
  <script type="text/javascript">
	...
	//查看供应商报价
	 function supplierView(supplierId){
		...
		location.href="${pageContext.request.contextPath}/adPackageExpert/supplierQuote.html?projectId="+projectId+"&supplierId="+supplierId;
	 }
	...
	function quoteAgain(projectId, packId, status){
	    if (status ==1 ) {
	    	...
	    	$("#tab-3").load("${pageContext.request.contextPath}/Adopen_bidding/changtotal.html?projectId="+projectId+"&packId="+packId + "&flowDefineId=${flowDefineId}" + "&count=1");
	    } else {
	    	$("#tab-3").load("${pageContext.request.contextPath}/Adopen_bidding/changmingxi.html?projectId="+projectId+"&packId="+packId + "&flowDefineId=${flowDefineId}" + "&count=1");
	    }
	}

	function showQuoteHistory(projectId, packId, data) {
		var json = {"timestamp":data};
		$("#tab-3").load("${pageContext.request.contextPath}/Adopen_bidding/viewChangtotalByPackId.html?projectId=" + projectId + "&packId=" + packId+ "&flowDefineId=${flowDefineId}", json);
	}
	
	function showQuoteHistoryMingxi(projectId, packId, data) {
		var json = {"timestamp":data};
		$("#tab-3").load("${pageContext.request.contextPath}/Adopen_bidding/viewMingxi.html?projectId=" + projectId + "&packId=" + packId+ "&flowDefineId=${flowDefineId}", json);
	}
	$(function(){
		for (var i = 1; i < 20; i++) {
			$(".p0" + i).addClass("hide");
		...
		<c:if test="${dd.code eq 'JZXTP' || dd.code eq 'DYLY'}">
	        <c:if test="${isEndPrice !=1 }">
				 ...
				 <select onchange="showQuoteHistory('${project.id}','${packId}',this.options[this.options.selectedIndex].value)">
						<c:if test="${empty treemap.value[0].dataList or fn:length(treemap.value[0].dataList) eq 1}">
			   ...
			   <div>
				 ...
				 <c:if test="${dd.code eq 'JZXTP' || dd.code eq 'DYLY'}">
			        	<c:if test="${listPackage.isEndPrice != 1 }">
			        	 ...
			        	 <c:if test="${listPackage.isEndPrice eq 1 }">
			        		<button class="btn">已结束唱标</button>
			        	</c:if>
			        	<span class="ml50">查看历史报价:</span>
						...
						<table id="${suList.id}"  class="table table-bordered table-condensed mt5">
							...
							<c:forEach items="${suList.quoteList}" var="quoteList" varStatus="vs">
										<tr id="${pdList.id }" class="hand">
											...
											<td class="tl">${quoteList.advancedDetail.goodsName}</td>
											<td class="tl">${quoteList.advancedDetail.stand}</td>
											<td class="tl w200">${quoteList.advancedDetail.qualitStand}</td>
											<td class="tc w50">${quoteList.advancedDetail.item}</td>
											<td class="tc w50">${quoteList.advancedDetail.purchaseCount}</td>
