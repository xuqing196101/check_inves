	...
	function eachTable(obj) {
		...
		if(error == 0) {
			var projectId = $("#projectId").val();
			 $.ajax({
			        type: "POST",
			        url: "${pageContext.request.contextPath}/Adopen_bidding/savemingxi.html?projectId="+projectId + "&packId=${packId}" + "&flowDefineId=${flowDefineId}",
			        data: {quoteList:JSON.stringify(jsonStr)},
			        dataType: "json",
			        success: function (message) {
			        	if ('${packId}' == null || '${packId}' == "") {
			        		window.location.href="${pageContext.request.contextPath}/Adopen_bidding/viewMingxi.html?projectId="+projectId;
			        	} else {
			        		$("#tab-3").load("${pageContext.request.contextPath}/adPackageExpert/toSupplierQuote.html?projectId=${projectId}&flowDefineId=${flowDefineId}");
			        	}
			        }
			...
			<c:forEach items="${listPackage.suppliers}" var="suList" varStatus="vs">
				<div class="p0${index}">
					...
					<table id="${suList.id}"  class="table table-bordered table-condensed mt5">
						...
						<c:forEach items="${suList.details}" var="pdList" varStatus="vs">
