...
<html class=" js cssanimations csstransitions" lang="en">
  ...
  <head>
    ...
    <script type="text/javascript">
      ...
      //预览
      function pre_view() {
        //$("#form").attr("action",'${pageContext.request.contextPath}/Adopen_bidding/printView.html');   
        //$("#form").submit();
      ...
      function publish() {
        ...
        if(categoryId != null && categoryId != "") {
          ...
          $.ajax({
            type: 'post',
            url: "${pageContext.request.contextPath}/Adopen_bidding/saveBidNotice.do?flag=1",
            ...
            success: function(result) {
              if(!result.success) {
                ...
                parent.window.setTimeout(function() {
                  if(noticeType == 'win') {
                    window.location.href = "${pageContext.request.contextPath}/Adopen_bidding/winNotice.html?projectId=" + result.obj.projectId + "&flowDefineId=" + flowDefineId;
                  } else if(noticeType == 'purchase') {
                    window.location.href = "${pageContext.request.contextPath}/Adopen_bidding/bidNotice.html?projectId=" + result.obj.projectId + "&flowDefineId=" + flowDefineId;
                  }
                }, 500);
                layer.msg(result.message, {
                  offset: ['220px']
        ...
        /* var iframeWin;
              layer.open({
               ...
               // skin: 'layui-layer-rim',
                ...
                offset: '100px',
                shadeClose: false,
                content: '${pageContext.request.contextPath}/Adopen_bidding/publishEdit.html?id='+articleId+'&noticeType='+noticeType+'&flowDefineId='+flowDefineId,
                success: function(layero, index){
                  iframeWin = window[layero.find('iframe')[0]['name']]; //得到iframe页的窗口对象，执行iframe页的方法：iframeWin.method();
                }
              }); */
      ...
      function save() {
        ...
        if(categoryId != null && categoryId != "") {
          $.ajax({
            type: 'post',
            url: "${pageContext.request.contextPath}/Adopen_bidding/saveBidNotice.do?flag=0",
            dataType: 'json',
            data: $('#form').serialize(),
            success: function(data) {
              if(!data.success) {
