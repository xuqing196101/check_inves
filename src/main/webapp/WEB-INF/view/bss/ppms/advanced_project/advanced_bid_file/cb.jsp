...
<html class=" js cssanimations csstransitions time_animations" lang="en">
  <!--<![endif]-->
    ...
    <script type="text/javascript">
      ...
      function showTime() {
        if('${date}' > 0) {
          ...
          $.ajax({
            url: "${pageContext.request.contextPath}/Adopen_bidding/showTime.do",
            type: "post",
            data: {
              projectId: projectId
            },
      ...
      function changtotal(obj) {
        ...
        $.ajax({
          ...
          success: function(data) {
            if(data == "1") {
              window.location.href = "${pageContext.request.contextPath}/Adopen_bidding/changtotal.html?projectId=" + projectId + "&flowDefineId=${flowDefineId}";
              return;
            } else if(data == "2") {
              layer.msg("报价已完成,且是唱明细", {
                offset: [y, x]
              });
              return;
            } else {
              layer.confirm('确认后将不可修改此次操作？', {
                  title: '提示',
                  offset: [y, x],
                  shade: 0.01
                },
