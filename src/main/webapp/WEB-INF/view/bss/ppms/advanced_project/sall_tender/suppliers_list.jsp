...
<html>
  ...
  <script type="text/javascript">
        ...
        jump: function(e, first) { //触发分页后的回调
          if(!first) { //一定要加此判断，否则初始时会无限刷新
              ...
              var projectId = "${projectId }";
        	    var path="${pageContext.request.contextPath}/adSaleTender/showAllSuppliers.html?packageId=" + packages + "&projectId=" + projectId+"&supplierName="+$("#supplierName").val()+"&page="+e.curr+"&ix=${ix}"+"&selectValue="+$("#hiddenValue").val();
                $("#tab-1").load(path);
    ...
    function query(){
        ...
        var packages = "${packageId }";
        var projectId = "${projectId }";
        var path="${pageContext.request.contextPath}/adSaleTender/showAllSuppliers.html?packageId=" + packages + "&projectId=" + projectId+"&supplierName="+$("#supplierName").val()+"&ix=${ix}";
            $("#tab-1").load(path);
    }
    ...
    function showSupplier() {
      ...
      if(kind == 'DYLY'){
        if(id.length == 1){
         var path = "${pageContext.request.contextPath}/adSaleTender/saveSupplier.html?ids=" + id + "&packages=" + packages + "&projectId=" + projectId+"&ix=${ix}";
        $.ajaxSetup({cache:false});
         $("#tab-1").load(path);
        }else{
    	 ...
    	 var idstr = $("#hiddenValue").val();
    	  var path  = "${pageContext.request.contextPath}/adSaleTender/saveSupplier.html?ids=" + idstr + "&packages=" + packages + "&projectId=" + projectId + "&ix=${ix}";
        $.ajaxSetup({cache:false});
        $("#tab-1").load(path);
      }
  ...
  <body>
    ...
    <input type="hidden" name="packages" value="${packageId }" />
