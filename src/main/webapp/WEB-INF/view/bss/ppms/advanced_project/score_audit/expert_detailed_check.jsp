...
<html>
  ...
  <script type="text/javascript">
				    		...
				    		$.ajax({
								...
								success:function(result){
								    	if(!result.success){
								    		...
								    		layer.close(index);
								    		$("#tab-6").load('${pageContext.request.contextPath}/adPackageExpert/checkAuditView.html?packageId='+packageId+'&projectId='+projectId);
								    	}
					                },
	...
	function openPrint(projectId,packageId){
		window.open("${pageContext.request.contextPath}/adPackageExpert/expertConsult.html?packageId="+packageId+"&projectId="+projectId+"&flag=1", "评审汇总表");
	}
	
	function openDetailPrint(projectId,packageId){
		window.open("${pageContext.request.contextPath}/adPackageExpert/openAllPrint.html?packageId="+packageId+"&projectId="+projectId+"&auditType=1", "打印所有评审数据");
	}
	
	function expertConsult(projectId,packageId){
		window.open("${pageContext.request.contextPath}/adPackageExpert/expertConsult.html?packageId="+packageId+"&projectId="+projectId, "专家咨询委员会");
	}
	...
	function refreshWin() {
		...
		var url = '${pageContext.request.contextPath}/adPackageExpert/checkAuditView.html?packageId='+packageId+'&projectId='+projectId;
		$("#tab-6").load(url);
	}
  </script>
  <body>
	    ...
	    <div class="mb5 fr">
	      ...
	      <c:forEach items="${packExpertExtList}" var="ext" varStatus="vs">
		       <tr>
		        ...
		        <td class="tc"><a href="${pageContext.request.contextPath}/adPackageExpert/printView.html?projectId=${projectId}&packageId=${pack.id}&expertId=${ext.expert.id}&auditType=1" target="view_window" title="评审明细">${ext.expert.relName}</a></td>
		        <c:forEach items="${supplierList}" var="supplier" varStatus="vs">
		        	<td class="tc">
		        	  	${ext.isPass}
		        	</td>
  		...
  		<div class="clear col-md-12 pl20 mt10 tc">
		    <button class="btn btn-windows back" onclick="goBack('${pageContext.request.contextPath}/adPackageExpert/toScoreAudit.html?projectId=${projectId}&flowDefineId=${flowDefineId}')" type="button">返回</button>
	   	</div>
  </body>
</html>
