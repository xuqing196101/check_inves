...
<script type="text/javascript">
	...
	function supplierView(supplierId) {
		...
		location.href = "${pageContext.request.contextPath}/adPackageExpert/supplierQuote.html?projectId="
	...
	function backScore(){
		...
		if (count == 0) {
			...
			$.ajax({
				url: "${pageContext.request.contextPath}/adPackageExpert/backScore.html?projectId=${projectId}&packageId=${packageId}&expertId=" + expertIds,
				async: true,
				success: function () {
					layer.alert("复核成功!", {
						offset : [ y, x ],
	...
	function showViewByExpertId(expertId){
		$.ajax({
			...
			success: function (response) {
				if (response == '1') {
					window.open("${pageContext.request.contextPath}/adPackageExpert/showViewByExpertId.html?projectId=${projectId}&packageId=${packageId}&expertId=" + expertId, "评分详情");
				} else {
					layer.alert("该专家暂未评分!", {
						offset : [ y, x ],
						shade : 0.01
	...
	function showViewBySupplierId(supplierId){
		window.open("${pageContext.request.contextPath}/adPackageExpert/showViewBySupplierId.html?projectId=${projectId}&packageId=${packageId}&supplierId=" + supplierId, "评分详情");
	}
	function printRank(){
		var packageId = "${packageId}";
		...
		if (isEnd != "1") {
			...
			window.open("${pageContext.request.contextPath}/adPackageExpert/printRank.html?packages=${packageId}", "打印汇总表");
		}
	}
</script>
</head>
<body>
  ...
  <div class="clear col-md-12 pl20 mt10 tc">
	<input type="button" class="btn btn-windows back" value="返回" onclick="goBack('${pageContext.request.contextPath}/adPackageExpert/toScoreAudit.html?projectId=${projectId}&flowDefineId=${flowDefineId}')">
  </div>
