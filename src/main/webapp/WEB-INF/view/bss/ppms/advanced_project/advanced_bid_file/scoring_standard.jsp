...
<script type="text/javascript">
    ...
    //${pageContext.request.contextPath}/intelligentScore/getMarkTermTree.do?packageId=A1CC4A603B6F466C936BBA2BA7BC2317
     ...
     var setting = {  
        async : {  
            ...
            url : "${pageContext.request.contextPath}/adIntelligentScore/getMarkTermTree.do?packageId=<%=packageId%>"+"&bidMethodId=<%=bidMethodId%>"+"&projectId=<%=projectId%>",  
            autoParam : ["id"],  
            dataFilter : filter,  
    ...
    function beforeRemove(treeId, treeNode) {  
    	...
    	layer.confirm(content, {title:'提示',offset: ['222px','360px'],shade:0.01}, function(index){
				$.post("${pageContext.request.contextPath}/adIntelligentScore/operatorNode.do",{id:treeNode.id,method:"delnode",packageId:packageId},
				function(){
    ...
    function beforeRename(treeId, treeNode, newName) {  
        ...
        $.post("${pageContext.request.contextPath}/adIntelligentScore/operatorNode.do",{id:treeNode.id,name:newName,method:"updatenode",packageId:packageId,projectId:projectId,remainScore:remainScore});  
        return true;  
    }  
  	function  beforeEditName(){
  		layer.open({
					  ...
					  area: ['500px', '20%'],
					  offset: '100px',
					  content: "${pageContext.request.contextPath}/adIntelligentScore/addNode.do?pid="+treeNode.id+"&method=addnode"+"&packageId="+packageId+"&projectId="+projectId+"&remainScore="+remainScore //iframe的url
				}); 
  	}
    ...
    function zTreeOnClick(event,treeId,treeNode){
		...
		if(treeNode.bidMethodId!=null && treeNode.bidMethodId!=""){
			...
			$("#treebody").attr("src","${pageContext.request.contextPath}/adIntelligentScore/gettreebody.do?id="+treeNode.id+"&packageId="+id+"&name="+encodeURI(encodeURI(treeNode.name))+"&projectId=${projectId}");
	...
	function setBidMothodForm(id,packageId){
		$.ajax({
			    type: 'post',
			    url: "${pageContext.request.contextPath}/adIntelligentScore/getBidMethodById.do",
			    dataType:'json',
	...
	function reList(projectId){
		window.location.href = "${pageContext.request.contextPath}/adIntelligentScore/packageList.do?projectId="+projectId;
	}
	function show(){
	...
	function save(){
		...
		$.ajax({
			    type: 'post',
			    url: "${pageContext.request.contextPath}/adIntelligentScore/operatorBidMethod.do",
			    dataType:'json',
