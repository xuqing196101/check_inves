	...
	function sum2(obj) { //数量
		...
		$.ajax({
			...
			success : function(data) {
				...
				var budgets = $(obj).parent().next().next().children(":last")
						.prev();
				$(budgets).val(sum);
				...
				$("#table tr").each(
						function() {
							var cid = $(this).find("td:eq(8)")
									.children(":last").val();
							var same = $(this).find("td:eq(8)").children(
									":last").prev().val() - 0;
							if (id == cid) {
				...
				for ( var i = 0; i < data.length; i++) {
					...
					$("#table tr").each(
							function() {
								var pid = $(this).find("td:eq(8)").children(
										":first").val();//上级id
								if (data[i].id == pid) {
									$(this).find("td:eq(8)").children(":first")
											.next().val(budget);
								}
	...
	function sum1(obj) {
		...
		$.ajax({
			...
			success : function(data) {
				...
				var price2 = $(obj).parent().prev().children(":last").prev()
						.val() - 0;//数量
...
<body>
	...
	<div class="margin-top-10 breadcrumbs ">
		<div class="container">
			<ul class="breadcrumb margin-left-0">
				...
				<li><a href="javascript:void(0)">项目管理</a>
				</li>
				<li class="active"><a href="javascript:void(0)">项目调整</a>
				</li>
			</ul>
			<div class="clear"></div>
		</div>
	...
	<div class="container container_box">
		<form action="${pageContext.request.contextPath}/project/update.html"
			...
			<div>
				...
				<ul class="ul_list">
					<li class="col-md-3 margin-0 padding-0 "><input type="hidden"
						id="ide" name="ide" value="${project.id}" /> <span
						class="col-md-12 padding-left-5">项目名称</span>
						<div class="input-append">
							<input type="text" id="jname" name="name" class="span5"
								value="${project.name}" /> <span class="add-on">i</span>
						</div></li>
					<li class="col-md-3 margin-0 padding-0 "><span
						...
						<div class="input-append">
							<input type="text" id="projectNumber" name="projectNumber"
								class="span5" value="${project.projectNumber}" /> <span
								class="add-on">i</span>
						</div></li>
				</ul>
			</div>
						...
						<table id="table"
							...
							<c:forEach items="${lists}" var="obj" varStatus="vs">
								<tr class="${obj.parentId}" style="cursor: pointer;">
									...
									<td class="tc"><c:if test="${obj.purchaseCount!=null }">
											<input type="hidden" name="ss" value="${obj.id }">
											<input maxlength="11" id="purchaseCount" onblur="sum2(this);"
												onkeyup="this.value=this.value.replace(/\D/g,'')"
												onafterpaste="this.value=this.value.replace(/\D/g,'')"
												name="lists[${vs.index }].purchaseCount" style="width:50%;"
												value="${obj.purchaseCount}" />
											<input type="hidden" name="ss" value="${obj.parentId }">
										</c:if> <c:if test="${obj.purchaseCount==null }">
											<input style="border: 0px;" disabled="disabled" type="text"
												name="lists[${vs.index }].purchaseCount"
												value="${obj.purchaseCount }">
										</c:if></td>
									<td class="tc"><c:if test="${obj.price!=null }">
											<input type="hidden" name="ss" value="${obj.id }">
											<input maxlength="11" id="price"
												name="lists[${vs.index }].price" style="width:50%;"
												onblur="sum1(this);" value="${obj.price}" />
											<input type="hidden" name="ss" value="${obj.parentId }">
										</c:if> <c:if test="${obj.price==null}">
											<input style="border: 0px;" readonly="readonly"
												onblur="sum1(this)" type="text"
												name="lists[${vs.index }].price" value="${obj.price }">
										</c:if></td>
									<td class="tc"><input type="hidden" name="ss"
										value="${obj.id }"> <input maxlength="11" id="budget"
										name="lists[${vs.index }].budget"
										style="width:50%;border-style:none" readonly="readonly"
										value="${obj.budget}" /> <input type="hidden" name="ss"
										value="${obj.parentId }"></td>

									<td class="tc">${obj.deliverDate}</td>
									<td class="tc advice"><c:if
