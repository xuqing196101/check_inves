...
function edit(){
	...
	if(id.length==1){
		window.location.href="<%=basePath%>productQuota/edit.do?id="+id+"&proId="+proId;
	}else if(id.length>1){
...
function add(){
	var proId = $("#proId").val();
	window.location.href="<%=basePath%>productQuota/add.html?proId="+proId;
}
...
function del(){
	...
	if(ids.length>0){
		layer.confirm('您确定要删除吗?', {title:'提示',offset: ['222px','360px'],shade:0.01}, function(index){
			layer.close(index);
			window.location.href="<%=basePath%>productQuota/delete.html?proId="+proId+"&ids="+ids;
		});
	}else{
