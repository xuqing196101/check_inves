...
function add(){
	var proId = $("#proId").val();
	window.location.href="<%=basePath%>periodCost/add.html?proId="+proId;
}
...
function del(){
	...
	if(ids.length>0){
		layer.confirm('您确定要删除吗?', {title:'提示',offset: ['222px','360px'],shade:0.01}, function(index){
			layer.close(index);
			window.location.href="<%=basePath%>periodCost/delete.html?proId="+proId+"&ids="+ids;
		});
	}else{
...
function nextStep(){
	...
	window.location.href="<%=basePath%>yearPlan/select.do?proId="+proId+"&total="+total;
}
