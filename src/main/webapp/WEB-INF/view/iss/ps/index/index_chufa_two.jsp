	  ...
	  laypage({
		    ...
		    jump: function(e, first){ //触发分页后的回调
		        if(!first){ //一定要加此判断，否则初始时会无限刷新
		            var url = "${pageContext.request.contextPath}/index/selectIndexChuFaNewsByTypeId.html?page="+e.curr+"&id="+articleTypeId+"&title="+title;
...
function query(){
	var title = $("#title").val();
	var url="${pageContext.request.contextPath}/index/selectIndexChuFaNewsByTypeId.html?id="+articleTypeId+"&title="+title;
	window.location.href = encodeURI(encodeURI(url));
}
</script>
</head>
  ...
  <!--面包屑导航开始-->
   <div class="margin-top-10 breadcrumbs">
      <div class="container">
		   ...
		   <li><a href="${pageContext.request.contextPath}/"> 首页</a></li><li><a href="javascript:void(0);">${typeName}</a></li>
		   </ul>
		<div class="clear"></div>
	  </div>
   </div>
