	    ...
	    jump: function(e, first){ //触发分页后的回调
	        if(!first){ //一定要加此判断，否则初始时会无限刷新
	            ...
	            var publishStartDate=$("#publishStartDate").val();
	      		...
	      		var url="${pageContext.request.contextPath}/index/selectAllByTabs.html?page="+e.curr
  				+"&articleTypeId="+articleTypeId+"&secondArticleTypeId="+secondArticleTypeId+"&title="+title+"&productType="+productType+"&productTypeName="+productTypeName+"&tab="+tab+"&lastArticleTypeName="+lastArticleTypeName+"&publishStartDate="+publishStartDate+"&publishEndDate="+publishEndDate;
...
function query(){
	...
	//alert(title);
	 ...
	 var url="${pageContext.request.contextPath}/index/selectAllByTabs.html?articleTypeId="+articleTypeId+"&secondArticleTypeId="+secondArticleTypeId+"&title="+title+"&productType="+productType+"&productTypeName="+productTypeName+"&tab="+tab+"&lastArticleTypeName="+lastArticleTypeName+"&publishStartDate="+publishStartDate+"&publishEndDate="+publishEndDate;
	 window.location.href = encodeURI(encodeURI(url));
}
function myReSet(){
	var tab="";
	var url="${pageContext.request.contextPath}/index/selectAllByTabs.html?articleTypeId="+articleTypeId+"&secondArticleTypeId="+secondArticleTypeId+"&tab="+tab;
	window.location.href = encodeURI(encodeURI(url));
}
       ...
       <div id="categoryContent" class="categoryContent" style="display:none; position: absolute;left:0px; top:0px; z-index:999;">
		...
		<div class="form-group">
			...
			<select name="pt" id="pt" class="form-control">
				<option value="物资" <c:if test="${'物资' eq tab }"> selected=selected </c:if> >物资</option>
				<option value="工程" <c:if test="${'工程' eq tab }"> selected=selected </c:if> >工程</option>
				<option value="服务" <c:if test="${'服务' eq tab }"> selected=selected </c:if> >服务</option>
				<option value="进口" <c:if test="${'进口' eq tab }"> selected=selected </c:if> >进口</option>
			</select>
		</div>
		...
		<div class="form-group" id="choseCategory" >
					<input id="cId" name="categoryId" type="hidden"
						...
						type="text" name="categoryName"  class="form-control" readonly value="${categoryNames}"
			...
			<div class="form-group">
				...
				<span>-</span><input class="form-control" readonly="readonly" onClick="WdatePicker({dateFmt:'yyyy-MM-dd'})"
