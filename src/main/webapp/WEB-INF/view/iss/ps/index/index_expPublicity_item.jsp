    ...
    <script type="text/javascript">
        $(function(){
            ...
            if(mat == "mat_page"){
                ...
                var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + matCodeId;
                $("#tbody_category").load(path);
            }else if(eng == "eng_page"){
                // 工程品目信息
                loading = layer.load(1);
                var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + engCodeId;
                $("#tbody_category").load(path);
            }else if(ser == "ser_page"){
                // 服务
                loading = layer.load(1);
                var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + serCodeId;
                $("#tbody_category").load(path);
            }else if(goodsProject == "goodsProject_page"){
                // 工程产品类别信息
                loading = layer.load(1);
                var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + goodsProjectId;
                $("#tbody_category").load(path);
            }else if(goodsEngInfo == "goodsEngInfo_page"){
                // 工程专业属性信息
                loading = layer.load(1);
                var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + goodsEngInfoId;
                $("#tbody_category").load(path);
            }
        });
        ...
        function showDivTree(code) {
            ...
            loading = layer.load(1);
            var expertId = $("#expertId").val();
            var path = "${pageContext.request.contextPath}/expertQuery/getCategories.do?flags=publicity&expertId=" + expertId + "&typeId=" + code;
            $("#tbody_category").load(path);
        };
    </script>
</head>
