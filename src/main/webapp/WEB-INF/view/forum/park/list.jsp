...
<html>
  ...
  <jsp:include page="/backhead.jsp"></jsp:include>
  	<div >
		<a href="<%=basePath%>park/add.do">新增</a>
	</div>
    <table >
		<thead>
			<tr>
		...
		<c:forEach items="${list}" var="park" varStatus="vs">
			<tr>
				<td >${park.id }</td>
				<td onclick="view(${park.id});">${park.name}</td>	
				<td>
					<a href="<%=basePath%>park/edit.do?id=${park.id}">修改</a>
					<a href="<%=basePath%>park/delete.do?id=${park.id}">删除</a>
				</td>
			</tr>
		</c:forEach>
	</table>
