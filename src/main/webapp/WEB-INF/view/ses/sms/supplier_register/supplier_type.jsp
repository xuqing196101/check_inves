														...
														<c:forEach
															...
															<tr
																...
																<td class="tc"
																	...
																	<c:if test="${currSupplier.status==-1 || fn:contains(audit,'supplierBank')}"> 	<u:show showId="pro_show_${certProNumber}"  delete="false" businessId="${certPro.id}" typeId="${supplierDictionaryData.supplierProCert}" sysKey="${sysKey}" /></c:if>
													...
													<c:forEach
														...
														<tr
															...
															<td class="tc"
																...
																<c:if test="${currSupplier.status==-1 || fn:contains(audit,'supplierBank')}"> 	<u:show showId="sale_show_${certSaleNumber}"     businessId="${certSell.id}" typeId="${supplierDictionaryData.supplierSellCert}" sysKey="${sysKey}" /> </c:if>
											...
											<c:forEach items="${rootArea}" var="area" varStatus="st">
												<li class="col-md-3 col-sm-6 col-xs-12 pl10" id="area_${area.id}" >
													...
													<div class="input-append col-md-12 col-sm-12 col-xs-12 input_group p0">
														...
														<c:if test="${currSupplier.status==-1 || fn:contains(audit,'supplierBank')}">  <u:show showId="area_show_${st.index+1}" businessId="${currSupplier.id}_${area.id}" sysKey="${sysKey}" typeId="${supplierDictionaryData.supplierProContract}" /></c:if>
														...
														<tr
															...
															<td class="tc"
																...
																<c:if test="${(fn:contains(servePageField,certSe.id)&&currSupplier.status==2 ) ||currSupplier.status==-1}">	 <u:upload
																	...
																	<c:if test="${currSupplier.status==-1 || fn:contains(audit,'supplierBank') }">	 <u:show showId="se_show_${certSeNumber}"   businessId="${certSe.id}" 	typeId="${supplierDictionaryData.supplierServeCert}" sysKey="${sysKey}" /> </c:if>
