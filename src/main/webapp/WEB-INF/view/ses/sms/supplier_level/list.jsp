...
<script type="text/javascript">
	...
	function changeStatus() {
		...
		var id = radio.val();
		var currText = $(radio).parents("tr").find("td").eq(7).text().trim();
		var status = "";
		if (currText == "已停用") {
			var status = "${status}";
			console.info(status);
				...
				<table class="table table-bordered table-condensed">
					...
					<tbody>
						<c:forEach items="${list}" var="stars" varStatus="vs">
							<tr>
								<td class="tc"><input name="radio" type="radio" value="${stars.id}"></td>
								...
								<td class="tc status">
									<c:if test="${stars.status == 0}">已停用</c:if>
									<c:if test="${stars.status == 1}">已启用</c:if>
								</td>
							</tr>
						</c:forEach>
					</tbody>
