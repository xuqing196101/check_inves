...
<html>
				...
				laypage({
					...
					jump: function(e, first) { //触发分页后的回调
						if(!first) { //一定要加此判断，否则初始时会无限刷新
							...
							var path = "${pageContext.request.contextPath}/supplierQuery/ajaxContract.html?supplierId=" + supplierId + "&supplierTypeId=" + type + "&pageNum=" + pageNum;
							if(type == "PRODUCT") {
								$("#tab-1").load(path);
							} else if(type == "SALES") {
								$("#tab-2").load(path);
	...
	<body>
		...
		<form id="formSearch" action="${pageContext.request.contextPath}/supplierQuery/ajaxContract.html">
