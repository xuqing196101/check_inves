...
<head>
    ...
    <script type="text/javascript"
            src="${pageContext.request.contextPath}/js/sms/supplier_query/select_ruku_supplier_by_province.js"></script>
    <script type="text/javascript"
            src="${pageContext.request.contextPath}/js/sms/supplier_query/select_supplier_common.js"></script>
    ...
    <script type="text/javascript">
            ...
            laypage({
                ...
                jump: function (e, first) { //触发分页后的回调
                    if (!first) { //一定要加此判断，否则初始时会无限刷新
                        ...
                        $("#form1").submit();
                        /* location.href = '
                        ${pageContext.request.contextPath}/supplierQuery/findSupplierByPriovince.do?page=' + e.curr + "&judge=5" + "&address="  + encodeURI(encodeURI('
                        ${address}')); */
                    }
                }
            });
        ...
        $(function () {
            /* var optionNodes = $("option");
             ...
             if("
            ${supplier.score}" == $(optionNodes[i]).val()) {
        ...
        <ul class="breadcrumb margin-left-0">
            ...
            <li class="active">
                <a href="javascript:void(0);"
                   onclick="jumppage('${pageContext.request.contextPath}/supplierQuery/findSupplierByPriovince.html?judge=5&sign=2')">入库供应商列表</a>
            </li>
        </ul>
        <div class="clear"></div>
    </div>
...
<div id="supplierGradeTreeContent" class="supplierTypeContent"
     style="display:none; position: absolute;left:0px; top:0px; z-index:999;">
            ...
            <c:forEach items="${listSupplier.list }" var="list" varStatus="vs">
                <tr>
                    ...
                    <td class="hand" title="${list.supplierName}">
                        <a href="javascript:void(0);"
                           onclick="jumppage('${pageContext.request.contextPath}/supplierQuery/essential.html?judge=5&supplierId=${list.id}&sign=${sign}')">
                    ...
                    <td class="">${list.supplierType}</td>
                        <%-- <td class="hand" title="${list.supplierType}">
                            <c:if test="${fn:length (list.supplierType) > 10}">${fn:substring(list.supplierType,0,10)}...</c:if>
                            <c:if test="${fn:length (list.supplierType) <= 10}">${list.supplierType}</c:if>
                        </td> --%>
                    <td class="tc">
