    ...
    <div class="list-unstyled padding-10 breadcrumbs-v3 div-center">
        ...
        <span>
          ...
          <c:if test="${saleTenderList.get(0).bidFinish > 1}">
              <a href="${pageContext.request.contextPath}/mulQuo/priceBuild.html?projectId=${project.id}"
                 class="img-v1">价格构成表</a>
              <span class="green_link">→</span>
          </c:if>
           <c:if test="${saleTenderList.get(0).bidFinish == 1}">
           ...
           <c:if test="${saleTenderList.get(0).bidFinish < 1}">
               <a href="javascript:void(0);" onclick="show('${saleTenderList.get(0).bidFinish}')"
                  class="img-v3">价格构成表</a>
               <span class="">→</span>
           </c:if>
      </span>
            <%--  <span>
          ...
          <c:if test="${saleTenderList.get(0).bidFinish > 3}">
              <a href="${pageContext.request.contextPath}/supplierProject/bidDocument.html?projectId=${project.id}"
                 class="img-v1">编制标书</a>
              <span class="green_link">→</span>
          </c:if>
          <c:if test="${saleTenderList.get(0).bidFinish < 3}">
        ...
        <c:if test="${saleTenderList.get(0).bidFinish > 4}">
            <a href="${pageContext.request.contextPath}/supplierProject/toBindingIndex.html?projectId=${project.id}"
               class="img-v1">绑定指标</a>
            <span class="green_link">→</span>
        </c:if>
        <c:if test="${saleTenderList.get(0).bidFinish < 4}">
