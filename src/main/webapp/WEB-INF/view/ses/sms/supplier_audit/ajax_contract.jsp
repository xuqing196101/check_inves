               ...
               <c:if test="${obj.secondNode == null || obj.secondNode ==''}">
                 ${obj.firstNode}
					...
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.oneContract))}">style="border: 1px solid #FF8C00;"</c:if>>
						<div class="w125">
				      <u:upload singleFileSize="300" maxcount="5"   id="${fileUp}${(vs.index + 1)*6-1}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.oneContract}" auto="true" />
					...
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.twoContract))}">style="border: 1px solid #FF8C00;"</c:if>>
						<div class="w125">
						   <u:upload singleFileSize="300" maxcount="5"  id="${fileShow}${(vs.index + 1)*6-2}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.twoContract}" auto="true" />
					...
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.threeContract))}">style="border: 1px solid #FF8C00;"</c:if>>
						<div class="w125">
						   <u:upload singleFileSize="300" maxcount="5"  id="${fileShow}${(vs.index + 1)*6-3}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.threeContract}" auto="true" />
					...
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.oneBil))}">style="border: 1px solid #FF8C00;"</c:if>>
						<div class="w125" >
							<u:upload singleFileSize="300" maxcount="5"  id="${fileShow}${(vs.index + 1)*6-4}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.oneBil}" auto="true" />
							<u:show showId="${fileShow}${(vs.index + 1)*6-4}" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.oneBil}" />
						</div>
					</td>
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.twoBil))}">style="border: 1px solid #FF8C00;"</c:if>>
					    <div class="w125" >
					    <u:upload singleFileSize="300" maxcount="5"  id="${fileShow}${(vs.index + 1)*6-5}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.twoBil}" auto="true" />
							<u:show showId="${fileShow}${(vs.index + 1)*6-5}" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.twoBil}" />
					    </div>
					</td>
					<td class="" <c:if test="${fn:contains(fileModifyField,obj.itemsId.concat(obj.threeBil))}">style="border: 1px solid #FF8C00;"</c:if>>
						<div class="w125">
						  <u:upload singleFileSize="300" maxcount="5"  id="${fileShow}${(vs.index + 1)*6-6}" multiple="true" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.threeBil}" auto="true" />
							<u:show showId="${fileShow}${(vs.index + 1)*6-6}" businessId="${obj.itemsId}" sysKey="${sysKey}" typeId="${obj.threeBil}" />
					    </div>
					</td>
						<td class="tc w50">
