    ...
    <script type="text/javascript">
      ...
      function downloadTable(str) {
        ...
        if(size == 0) {
          ...
          var state = $("#" + id + "").parent("tr").find("td").eq(11).text(); //.trim();
          state = trim(state);
          if(state == "初审通过" || state == "初审未通过" || state == "退回修改" || state == "复审通过" || state == "复审不通过" || state == "复查通过" || state == "复查未通过") {
            ...
            <li>
              ...
              <select name="status" class="w178" id="status">
                ...
                <c:if test="${sign == 2}">
                  ...
                  <option <c:if test="${state eq '5'}">selected</c:if> value="5">复审不通过</option>
                </c:if>
                <c:if test="${sign == 3}">
            ...
            <tr>
              ...
              <td class="tl" onclick="shenhe('${expert.id}');" title="${expert.relName}">
                ...
                <c:if test="${sign == 2 and expert.status eq '-2' }">
                  <td class="tc"><span class="label rounded-2x label-dark" onclick="shenhe('${expert.id}');">复审预通过</span></td>
                </c:if>
                ...
                <c:if test="${sign == 2 and expert.status eq '5' }">
                  <td class="tc"><span class="label rounded-2x label-dark" onclick="shenhe('${expert.id}');">复审不通过</span></td>
                </c:if>
