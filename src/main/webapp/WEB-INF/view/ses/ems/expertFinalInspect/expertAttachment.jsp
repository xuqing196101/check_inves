						      ...
						      <tr>
							      <td class="tc">${vs.index + 1}</td>
								    ...
								    <td class="tc pl20" >
								    	...
								    	<c:if test="${expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>
								    ...
								    <td class="tl pl20">
										<input type="text" id="reason${list.id}" class="w100p mb0" value="${list.reason}" onblur="updReason('${list.id}','${list.typeId}')" <c:if test="${expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>>
									</td>
						      </tr>
						    </c:forEach>
						  </table> 
						...
						<h2 class="count_flow"><i>2</i>复查意见</h2>
						 <ul class="ul_list">
		                   ...
		                   <div class="select_check">
		                      <input type="radio" id="fchg" <c:if test="${auditOpinion.flagAudit eq '7'}">checked</c:if> name="status" value="7" onclick = "updStatus()" <c:if test="${qualified eq 'false'||expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>>复查合格
		                      <input type="radio"  id="fcbhg" <c:if test="${auditOpinion.flagAudit eq '8'}">checked</c:if> name="status" value="8" onclick = "updStatus()" <c:if test="${expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>>复查不合格
		                      <input type="radio"  id="zlbq" <c:if test="${auditOpinion.flagAudit eq '17'}">checked</c:if> name="status" value="17" onclick = "updStatus()" <c:if test="${qualified eq 'false'||expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>>资料不全
		                    </div>
		                  </li>
		                  <li>
		                  ...
		                  <li class="mt10">
		                     <textarea id="opinion" class="col-md-12 col-xs-12 col-sm-12 h80" onblur="updExpertReason()" <c:if test="${expert.status=='7' || expert.status=='8' || (expert.status=='17'&&notCount<=expert.finalInspectCount)}">disabled="disabled"</c:if>>${auditOpinion.opinion }</textarea>
		                  </li>
		                </ul>
					</div>
					<div class="clear"></div>
