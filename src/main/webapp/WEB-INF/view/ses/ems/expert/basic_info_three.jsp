...
<form id="formExpert" action="${pageContext.request.contextPath}/expert/add.html" method="post">
    ...
    <div id="reg_box_id_5" class="container clear margin-top-30 yinc">
        <h2 class="step_flow">
            ...
            <span id="ty6" class="new_step current fl" onclick='operation(6)'><i class="">3</i><div class="line"></div> <span class="step_desc_02">参评类别</span> </span>
                ...
                <c:forEach items="${allPurList}" var="org1" varStatus="vs">
                    <c:if test="${org1.cityId eq expert.address}">
                        ...
                        <tr <c:if test="${fn:contains(org1.shortName, '北京')}"> style="dispaly:none" </c:if>>
                            <td class="tc"><input  name="procurementDepId" type="radio" value="${org1.id}" onclick="checkDep(this)" <c:if test="${-1!=expert.status || org1.id=='6bca3c841a2b49819727e7516254e9e6'}"> disabled="disabled" </c:if>
