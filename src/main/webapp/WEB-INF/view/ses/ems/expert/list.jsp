    ...
    <script type="text/javascript">
      ...
      function creadible() {
        ...
        if(id == null) {
          ...
          if(state == "复审通过" || state == "复审不通过" || state == "待复查" || state == "复查通过" || state == "复查不通过") {
            index = layer.open({
              type: 2, //page层
              area: ['700px', '440px'],
              title: '诚信登记',
          ...
          <c:forEach items="${result.list }" var="e" varStatus="vs">
            <tr class="pointer">
              ...
              <td class="tc" id="${e.id}">
                ...
                <c:if test="${e.status eq '-3' and e.isProvisional eq '0'}">
                  <span class="label rounded-2x label-u">公示中</span>
                </c:if>
                <c:if test="${e.status eq '4' and e.isProvisional eq '0'}">
                  <span class="label rounded-2x label-u">复审通过</span>
                </c:if>
                <c:if test="${e.status eq '5' }">
                  <span class="label rounded-2x label-dark">复审不通过</span>
                </c:if>
