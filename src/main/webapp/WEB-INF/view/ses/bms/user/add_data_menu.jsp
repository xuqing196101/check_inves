	...
	<script type="text/javascript">
        ...
        $(document).ready(function(){
         var index = layer.load(0, {
			...
			$.ajax({
             type: "GET",
             async: false, 
             url: "${pageContext.request.contextPath}/preMenu/dataTree.do?userId="+userId,
             dataType: "json",
             success: function(zNodes){
         	  ...
         	  var userId = "${uid}";
	    	  $.ajax({
				url: "${ pageContext.request.contextPath }/preMenu/findDataSelect.do?userId="+userId,
		...
		function ajaxSubmit(uid,idstr){
			$.ajax({
             type: "POST",
             async: false, 
             url: "${pageContext.request.contextPath}/user/saveUserDate.do?userId="+uid+"&ids="+idstr,
             dataType: "JSON",
             success: function(str){
		        	 var index = layer.getFrameIndex(window.name); //先得到当前iframe层的索引
				     parent.layer.msg(str.msg,{offset: '222px'});
