...
(function ($) { 
	$(function(){
		...
		var $wrap = $('#uploader'),
	         ...
	         state = 'pending',
	         percentages = {},
	         mutiple = transBoolean($("#multipleId").val()),
	         extension = $("#extensionId").val(),
	         mimeTypes = $("#mimeTypesId").val(),
	         duplicate = transBoolean($("#duplicateId").val()),
	         auto = transBoolean($("#autoId").val()),
        // 判断浏览器是否支持图片的base64
		...
		uploader.on( 'uploadSuccess', function(file,res) {
		   	$.post(globalPath + '/file/finished.html'
		   			,{fileName: file.name, path: res._raw , businessId: $("#businessId").val(),
		   			  typeId: $("#typeId").val(), key: $("#sysKeyId").val() ,mutiple: mutiple
		   			},
		   			function(msg){
		   				if (msg == 'ok') {
		   					$( '#'+file.id ).find('h4').text("上传成功");
		   					setTimeout(function(){$list.empty();}, 1000);
		   					display({businessId: $("#businessId").val(),
		   			                typeId: $("#typeId").val(), key: $("#sysKeyId").val()});
		   				}
