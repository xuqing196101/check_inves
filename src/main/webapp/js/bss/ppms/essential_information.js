...
function loadUploadData(data, i, relName){
    var html = "<tr> "
            ...
            + " <td><a href='javascript:void(0)' onclick=\"views('"+data.typeId+"','"+data.id+"')\">"+data.name+"</a></td>"
            + " <td class='tc' onclick=\"views('"+data.typeId+"','"+data.id+"')\">"+relName+"</td>"
            + " <td class='tc'>"+timestampToDate('yyyy-MM-dd hh:mm:ss', data.createDate)+"</td>"
            html += "</tr>";
