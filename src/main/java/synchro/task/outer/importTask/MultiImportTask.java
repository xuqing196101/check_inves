		  ...
		  fileRepeater.initFiles();
	        ...
	        if (file != null && file.exists()){
	            ...
	            for (File f : files) { 
	                ...
	                if(StringUtils.isNotBlank(result)){
	                  ...
	                  if (f.getName().equals(Constant.PRODUCT_FILE_EXPERT)){
	                    ...
	                    for (File file2 : f.listFiles()) {
	                        ...
	                        if (file2.getName().contains(FileUtils.C_OB_PRODUCT_FILENAME)
	              ...
	              //竞价供应商导入
	                result = DictionaryDataUtil.getId(Constant.DATE_SYNCH_BIDDING_SUPPLIER);
	                if(StringUtils.isNotBlank(result)){
	                  ...
	                  if (f.getName().equals(Constant.SUPPLIER_FILE_EXPERT)){
	                	  ...
	                	  for (File file3 : f.listFiles()) {
	                		  if (file3.isDirectory()) {
								if (file3.getName().contains(FileUtils.C_OB_SUPPLIER_FILENAME)
										|| file3.getName().contains(FileUtils.M_OB_SUPPLIER_FILENAME)) {
	                ...
	                //竞价信息导入
	                result = DictionaryDataUtil.getId(Constant.DATA_TYPE_BIDDING_CODE);
	                if(StringUtils.isNotBlank(result)){
	                	/**竞价信息导入  只能是外网导入内网**/
	           ...
	           if(StringUtils.isNotBlank(result)){
					/** 产品库管理导入  **/
						...
						if (f.getName().equals(Constant.OUTER_FILE_PRODUCT_LIBRARY_EXPERT)) {
							...
							for (File file2 : f.listFiles()) {
								if (file2.isDirectory()) {
									OperAttachment.moveToPathFolder(
													file2,
													FileUtils.BASE_ATTCH_PATH + FileUtils.SUPPLIER_ATTFILE_PATH);
								}
							 }
							}
						}
	       ...
	       //资料 管理 导入
		   result = DictionaryDataUtil.getId(Constant.SYNCH_TEMPLATE_DOWNLOAD);
		   if(StringUtils.isNotBlank(result)){
				/** 门户模板管理  只能是外网导入 **/
