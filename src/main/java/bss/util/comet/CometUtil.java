...
public class CometUtil extends ConnectListener implements
  ...
  public boolean handleEvent(ConnectEvent connEvent) {
    ...
    System.out.println(conn.getRequest().getSession().getAttribute("currentUserId")+"----");
    Object userId = conn.getRequest().getSession()
  ...
  public void pushTo(Comet comet) {
    try {
      ConnectEvent connEvent = (ConnectEvent) CacheManager.getContent(
          comet.getUserId()).getValue();
      final CometConnection conn = connEvent.getConn();
      // 建立连接和用户的关系
      ...
      // 推送到指定的客户端
      engine.sendTo(Constant.CHANNEL_MSGCOUNT, engine.getConnection(connId),
          comet.getMsgCount());
      engine.sendTo(Constant.CHANNEL_MSG_DATA, engine.getConnection(connId),
          comet.getMsgData());
    } catch (Exception e) {
      // TODO: handle exception
      System.out.println(e.getMessage());
