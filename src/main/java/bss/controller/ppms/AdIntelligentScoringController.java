...
public class AdIntelligentScoringController extends BaseController{
	 ...
	 */
	@RequestMapping("list")
	public String list(@ModelAttribute AdvancedPackages packages,Model model,HttpServletRequest request){
		HashMap<String, Object> map = new HashMap<String, Object>();
		map.put("id", packages.getId());
		AdvancedPackages packages2 = ((packageService.findPackageAndBidMethodById(map)!=null && packageService.findPackageAndBidMethodById(map).size()>0)?packageService.findPackageAndBidMethodById(map).get(0):new AdvancedPackages());
		...
		return "bss/ppms/advanced_project/advanced_bid_file/scoring_standard";
	}
	...
	public AjaxJsonData operatorNode(@ModelAttribute MarkTerm markTerm,HttpServletRequest request){
		...
		if(method!=null && !method.equals("") && method.equals("addnode")){
			...
			if(markTerm.getBidMethodId()!=null && !markTerm.getBidMethodId().equals("")){
				...
				bidMethodService.delBidMethodByMap(map);
				map.clear();
				AdvancedPackages pack = new AdvancedPackages();
	...
	public String packageListCn(@ModelAttribute AdvancedPackages packages,Model model,HttpServletRequest request){
		HashMap<String,Object> map = new HashMap<String,Object>();
		map.put("projectId", packages.getProjectId());
		List<AdvancedPackages> packagesList = packageService.selectByAll(map);
		model.addAttribute("packagesList", packagesList);
		model.addAttribute("projectId", packages.getProjectId());
	...
	public String addNode(@ModelAttribute MarkTerm markTerm,Model model,HttpServletRequest request){
		...
		return "bss/ppms/advanced_project/advanced_bid_file/operator_node";
	}
	/**
	 * 
	 *〈简述〉
	...
	public void setPackageMarkTermTree(String packageId,String method,MarkTerm markTerm){
		HashMap<String, Object> map = new HashMap<String,Object>();
		map.put("id", packageId);
		AdvancedPackages p = packageService.selectByAll(map).get(0);
	...
	public List<SupplyMark> getScoreByModelThree(ScoreModel scoreModel,ArrayList<SupplyMark> supplyMarkList){
		if(supplyMarkList!=null && supplyMarkList.size()>0){
			Collections.sort(supplyMarkList, new SortByParam1());
...
class SortByParam1 implements Comparator {
